$Title  final project arrangement_add_advertisment
$Ontext
...
$Offtext
  ...
  Table ratio(k,j)  ratio of seat sold of film j at different timeslot k in cases
                          a          b         c        d         e       f
        morning         0.55       0.60      0.67      0.23     0.54    0.12
        noon            0.65       0.60      0.74      0.30     0.65    0.23
        afternoon       0.70       0.70      0.78      0.32     0.65    0.34
        evening         0.75       0.80      0.91      0.43     0.76    0.45
        night           0.93       0.90      0.95      0.56     0.78    0.56
        midnight        0.85       0.60      0.88      0.54     0.88    0.45  ;
  ...
  revenue ..        z  =e=  0.9*sum((i,j,k), a(i)*price(k,j)*ratio(k,j)*x(i,j,k))+sum(k,average_price_ratio*average_price*retail(k)*sum((i,j),ratio(k,j)*a(i)*x(i,j,k)))- sum(i,cost(i)*sum((j,k),x(i,j,k))) ;
  ...
  Model arrangement_add_advertisment  /all/ ;
...
rhsrng all
$offecho
  arrangement_add_advertisment.OptFile=1;

  solve arrangement_add_advertisment  maximizing z using mip;
